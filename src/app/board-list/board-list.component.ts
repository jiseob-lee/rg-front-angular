...
export class BoardListComponent implements OnInit, OnDestroy {
  ...
  routeEvent(router: Router) {
    router.events.subscribe((e) => {
      if (e instanceof NavigationEnd) {
        ...
        if (
          this.routerArray[this.routerArray.length - 1] == 'ko' ||
          this.routerArray[this.routerArray.length - 1] == 'en'
        ) {
  ...
  ngOnInit() {
    ...
    this.lang =
      this.route.snapshot.paramMap.get('lang') == null
        ? ''
      ...
      .subscribe((res) => {
        ...
        for (var i = 0; i < this.manageBoardList.length; i++) {
          if (this.manageBoardList[i].articleCount == 0) {
            ...
            this.manageBoardObject[this.manageBoardList[i].boardIdx] =
              this.manageBoardList[i].boardName;
  ...
  getManageBoardList(): void {
    this.environmentService.getManageBoardList(this.lang).subscribe(
      (response) => {
  ...
  getBoardListCount(boardNo: number): void {
    this.boardService.getBoardListCount(boardNo).subscribe(
      (response) => {
        //this.ngZone.run(() => {
        //setTimeout(() => {
      ...
      () => {
        ...
        this.listCount.subscribe((value) => {
          ...
          this.linkStart =
            Math.floor((this.pageNo - 1) / this.pageLinkCount) * this.pageLinkCount + 1; // linkStart
          this.linkEnd =
            pageEnd - this.linkStart > this.pageLinkCount
              ? this.linkStart + this.pageLinkCount
              : pageEnd;

          this.linkArray = Array.from(
            new Array(this.linkEnd - this.linkStart + 1),
            (x, i) => i + this.linkStart,
          );
          this.cdr.detectChanges();
